...
describe('src > index', () => {
    ...
    test('Not existing conf files should produce an error', () => {
        process.env = {
            ...process.env,
            CONF_FILES: 'override_empty',
        }

        expect(() => {
            config.load()
        }).toThrow(
            `Config error: Couldn't find or read file ${process.cwd()}/conf/override_empty.yml.`
        )
    })
    ...
    test('It throws an error when number cast on env value fails', () => {
        ...
        expect(() => {
            config.load()
        }).toThrow('Config error: expected a number got NotANumber')
    })
    ...
    test('It throws an error when boolean string cast on env value fails', () => {
        ...
        expect(() => {
            config.load()
        }).toThrow('Config error: expected a boolean got NotABoolean')
    })
    ...
    test('It throws an error when boolean number cast on env value fails', () => {
        ...
        expect(() => {
            config.load()
        }).toThrow('Config error: expected a boolean got 42')
    })
...
-------------------^`
    ...
    test('It throws an error when env file is not yaml valid', () => {
        process.env = {
            ...process.env,
            CONF_DIR: 'test/conf/malformatted_env_file',
            CONF_FILES: 'dev',
        }
        expect(() => {
            config.load()
...
--------------------^`
    ...
    test('It throws an error when no base file', () => {
        ...
        expect(() => {
            config.load()
        }).toThrow("Config error: Couldn't find or read file test/conf/no_base_file/base.yml.")
    })
